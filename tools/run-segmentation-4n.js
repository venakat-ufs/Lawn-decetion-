...
// Usage: node tools/run-segmentation-4n.js <input-png> [min_component_size]
(async function(){
  try {
    const input = process.argv[2] || 'test-screenshots/ml-03-two-polygons.png';
    const minSize = Number(process.argv[3]) || 80; // component pixel min
    ...
    console.log('Components found (4-n, >= ' + minSize + ' px):', components.length);
    ...
    const maskPath = path.resolve(path.dirname(p), 'seg-mask-4n.png');
    ...
    const jsonPath = path.resolve(path.dirname(p), 'seg-polygons-4n.json');
